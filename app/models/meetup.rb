...
class Meetup < ActiveRecord::Base
  ...
  validates_acceptance_of :date_and_time,
    if: Proc.new { |meetup| meetup.date_and_time && meetup.date_and_time.past? }, message: I18n.t('meetup.datetime_must_future')
  validates_acceptance_of :finish_date_and_time,
    if: Proc.new { |meetup| meetup.finish_date_and_time && meetup.finish_date_and_time < meetup.date_and_time },
    message: I18n.t('meetup.finish_after_start')
  validates :letter_subject, presence: true
  validates :letter_body, presence: true

  scope :active, -> { where('date_and_time > ? AND cancelled = ?', Time.new, false) }
