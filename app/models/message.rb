class Message
  ...
  validates_each :recipient_group do |model, attribute, value|
    filters = UsersFilter.collection.keys
    model.errors.add attribute, I18n.t('admin.messages.invalid_user_group')  if not value or value.detect { |el| not filters.include?(el) }
  end
