class User < ActiveRecord::Base
  ...
  def apply_omniauth(omniauth)
    if omniauth.is_a?(Hash) && omniauth.include?('user_info')
    ...
    if omniauth.is_a?(Hash) && omniauth.include?('provider') && omniauth.include?('uid')
      user_tokens.build(:provider => omniauth['provider'], :uid => omniauth['uid'])
    end
  end

  def self.create_based_omniauth(omniauth)
    unless omniauth['user_info']['email'].empty?
